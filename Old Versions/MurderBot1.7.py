...
async def set_perm_nick(ctx, member:discord.Member, *, nickname:str):
    ...
    with open('permanent.txt', "a") as fd:
...
def check_perm_nickname(user:int) -> bool:
    with open('permanent.txt', "r") as fd:
        lines = fd.readlines()
        for i, line in enumerate(lines):
...
async def clear_perm_nick(ctx, member:discord.Member):
    ...
    if check_perm_nickname(user):
        ...
        perm_nicknames_tasks.pop(perm_nicknames.index(user))
    ...
    with open('permanent.txt', "r") as fd:
        ...
        for i, line in enumerate(lines):
            if str(user) in line:
                ...
                break
        fd.close()
    with open('permanent.txt', "w") as fd:
        for line in lines:
            fd.write(line)
        fd.close()
...
async def nickAlt(member:discord.Member, nicknames, time):
...
@client.command()
async def set_alt_nick(ctx, member:discord.Member, nick1:str, nick2:str, time:float = 1):
    user = member.id
    nicknames = []
    nicknames.append(nick1)
    nicknames.append(nick2)

    if str(user) in str(alt_nicknames):
        await ctx.send(f"{member.name} already has an alternating nickname")
        return

    alt_nicknames.append(user)
   
    alt_nicknames_tasks.append([user, asyncio.create_task(nickAlt(member, nicknames, time))])

    await ctx.send(f"Changing {member.name}'s nickname!")

@client.command()
async def clear_alt_nick(ctx, member:discord.Member):
    user = member.id
    index = alt_nicknames.index(user)
    if str(user) in str(alt_nicknames):
        alt_nicknames_tasks[index][1].cancel()
        alt_nicknames_tasks.pop(index)
        alt_nicknames.remove(user)
        await ctx.send(f"Cleared {member.name}'s alternating nickname!")

    else:
        await ctx.send(f"{member.name} does not have an alternating nickname!")
        return
