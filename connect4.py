...
class Connect4(commands.Cog):
    ...
    async def on_reaction_add(self, reaction:discord.Reaction, user:discord.User):
        ...
        game_instance:Connect4Game = Connect4.games.get(reaction.message.id)
    ...
    class Connect4Game(Games):
        ...
        async def cleanup(self):
            ...
            if self.game_message.id in Connect4.games:
                game:Connect4Game = Connect4.games[self.game_message.id]
                #print(time() - game.lastInteractionTime)
                if time.time() - game.lastInteractionTime > 300: # OR game has finished (set a flag)
                    print("Removing emojis")
                    del Connect4.games[self.game_message.id]
                    return True
        
        async def timeout_cleanup(self, custom_id):
            if custom_id in Connect4.games:
                game:Connect4Game = Connect4.games[custom_id]
                #print(time() - game.lastInteractionTime)
                if time.time() - game.lastInteractionTime > 300: # OR game has finished (set a flag)
                    print("Removing emojis")
                    await game.cleanup()
                    del Connect4.games[custom_id]
                    return True
            return False
        ...
        class Connect4View(discord.ui.View):
            ...
            async def acceptButton(self, interaction: discord.Interaction, button: discord.ui.Button):
                ...
                Connect4.games[self.game_instance.game_message.id] = self.game_instance
